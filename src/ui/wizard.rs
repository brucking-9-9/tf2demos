//! `tf2demos review` — the one-card-at-a-time labelling wizard (HANDOFF §4 "Review wizard").
//!
//! All state lives in [`Session`] (display-free, tested); this module only draws it and maps
//! keys. Keys when no text box has focus: `1`–`9` toggle a label · `t` type a label · `c` cycle
//! class · `r` then `1`–`5` rating · `l` streak · `p` play at tick · `s` skip · `Enter` save (or
//! play when no label yet) · `Esc` quit (or leave the tab when embedded in the manager).
...
pub fn run_review(cfg: &Config, theme: &Theme) -> Result<()> {
    ...
    eframe::run_native(
        ...
        Box::new(move |cc| {
            ...
            Ok(Box::new(ReviewApp::new(session, palette, false)))
        }),
    )
...
pub(super) struct ReviewApp {
    session: Session,
    /// Inside the manager window: Esc leaves the tab instead of closing the window.
    embedded: bool,
    close_requested: bool,
    /// Set after every save/skip so an embedding app can reload its view of the index.
...
impl ReviewApp {
    pub(super) fn new(session: Session, p: Palette, embedded: bool) -> Self {
    ...
    /// True once (per request) after Esc / the close button when embedded.
    pub(super) fn take_close(&mut self) -> bool {
    ...
    fn pill(&self, ui: &mut egui::Ui, text: String, selected: bool) -> egui::Response {
        super::pill(ui, &self.p, &text, selected)
    }

    fn section(&self, ui: &mut egui::Ui, title: &str, hint: &str) {
        super::section(ui, &self.p, title, hint)
    }
    ...
    fn done_body(&mut self, ui: &mut egui::Ui) {
...
impl ReviewApp {
    /// Draw the wizard into `ui` (the whole window, or one tab of the manager).
